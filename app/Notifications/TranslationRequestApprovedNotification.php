...
class TranslationRequestApprovedNotification extends BaseNotification implements ShouldQueue
{
    ...
    {
        return __("Get notified when a translation has been approved by a reviewer");
    }
    ...
    public function toMail(User $notifiable)
    {
        ...
        if ($this->translationRequest->isComplete()) {
            ...
            $body = __(':userName has approved the :languageName translation for :sourceTitle, and it is officially completed.', [
            ...
            $body = __(':userName has approved the :languageName translation for :sourceTitle. :numReviewsRemaining.', [
                'userName' => '**' . $this->reviewer->name . '**',
                'languageName' => '**' . $this->translationRequest->language->name . '**',
                'sourceTitle' => '**' . $this->translationRequest->source->title . '**',
                'numReviewsRemaining' => trans_choice('[1] :count review remaining|[*] :count reviews remaining', $this->translationRequest->num_approvals_remaining),
            ]);
        }
