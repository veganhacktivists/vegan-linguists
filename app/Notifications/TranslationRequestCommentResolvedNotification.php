...
class TranslationRequestCommentResolvedNotification
    ...
    implements ShouldQueue
{
    ...
    {
        return __(
            'Get notified when a translator resolves a comment on a translation'
        );
    }
    ...
    public function toMail(User $notifiable)
    {
        ...
        $route =
            $notifiable->id === $source->author_id
                ...
                : route('translate', [
                    ...
                    $source->slug,
                    '#discussion',
                ]); // notifying a translator or reviewer

        $mail = (new MailMessage())
            ->subject(
                trans_choice(
                    '[1] Comment Resolved|[*] Comments Resolved',
                    $this->comments->count()
                )
            )
            ->line(
                __(
                    ':userName has resolved :numComments on the :languageName translation for :sourceTitle.',
                    [
                        ...
                        'numComments' => trans_choice(
                            '[1] a comment|[*] :count comments',
                            $this->comments->count()
                        ),
                    ]
                )
        ...
        $mail->action(__('View translation request'), $route);
