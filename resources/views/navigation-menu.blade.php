...
<header x-data="{ open: false }" x-init="$refs.mobileMenu.classList.remove('hidden')"
            ...
            <div class="flex items-center space-x-8">
                <span class="inline-flex">
                    <a href="{{ route('notifications') }}"
                        class="-mx-1 bg-white p-1 rounded-full text-gray-400 hover:text-gray-500 flex">
                        <span class="sr-only">{{ __('View notifications') }}</span>
                        <x-heroicon-o-bell class="h-6 w-6" />
                        @if ($numUnreadNotifications > 0)
                            <x-badge color="indigo" x-data=""
                            ...
                            <x-jet-dropdown align="right" width="48">
                                ...
                                <x-slot name="content">
                                    ...
                                    <form method="POST" action="{{ route('switch-user-mode') }}">
                                        ...
                                        <x-jet-dropdown-link href="{{ route('switch-user-mode') }}" onclick="event.preventDefault();
    ...
    <div x-show="open" x-ref="mobileMenu" x-transition:enter="transition-opacity ease-linear duration-300"
        ...
        x-transition:leave-end="opacity-0" class="fixed inset-0 z-40 md:hidden hidden" role="dialog" aria-modal="true">
        ...
        <nav x-show="open" x-transition:enter="transition ease-out duration-150 sm:ease-in-out sm:duration-300"
            ...
            <div class="max-w-8xl mx-auto py-3 px-2 sm:px-4 space-y-1">
                ...
                @else
                    ...
                    <x-jet-responsive-nav-link
                        href=" {{ route('home', ['filter' => 'unclaimed'] + \Request::all()) }}"
            ...
            <div class="border-t border-gray-200 pt-4 pb-3">
                <div class="max-w-8xl mx-auto px-4 flex items-center sm:px-6">
                    ...
                    <a href="{{ route('notifications') }}"
                        class="ml-auto flex-shrink-0 bg-white p-2 text-gray-400 hover:text-gray-500 flex">
                        <span class="sr-only">{{ __('View notifications') }}</span>
                        <x-heroicon-o-bell class="h-6 w-6" />
                        @if ($numUnreadNotifications > 0)
                            <x-badge color="indigo" x-data=""
                ...
                <div class="mt-3 max-w-8xl mx-auto px-2 space-y-1 sm:px-4">
                    ...
                    <form method="POST" action="{{ route('switch-user-mode') }}">
                        ...
                        <x-jet-responsive-nav-link href="{{ route('switch-user-mode') }}" onclick="event.preventDefault();
                                                             this.closest('form').submit();">
                            @if (Auth::user()->isInAuthorMode())
                                {{ __('Switch to Translator view') }}
                            @else
